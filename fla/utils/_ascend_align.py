...
def npu_leftover_mask(
    *,
    ...
    Uses ``n % tile != 0``, not ``n % 32 != 0``. When ``n % tile == 0`` every
    tile is full; leftover DMA rounded to 32 is absorbed by ``npu_pad``
    (e.g. n=48, tile=16 pads to 96). Gating ``tl.where`` on ``n % 32`` would
    force leftover copies — and smaller Cube tiles — on complete-tile shapes.
    """
    if varlen:
        return True
    return any(
