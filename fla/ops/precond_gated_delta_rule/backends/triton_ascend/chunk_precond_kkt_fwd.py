...
def chunk_precond_kkt_fwd_kernel_npu(
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        if i_t * BT + 1 < T:
            b_b = tl.load(beta + i_hv * bt_stride + bos + o_t, mask=m_t, other=0.0).to(tl.float32)
            b_g = tl.load(g + i_hv * bt_stride + bos + o_t, mask=m_t, other=0.0).to(tl.float32)
            ...
            for i_k in range(tl.cdiv(K, BK)):
                ...
                b_kp = tl.load(
                    k_precond + (bos * H + i_h) * K + o_t[:, None] * (H * K) + o_k[None, :],
                    mask=m_tk, other=0.0,
                ).to(tl.float32)
