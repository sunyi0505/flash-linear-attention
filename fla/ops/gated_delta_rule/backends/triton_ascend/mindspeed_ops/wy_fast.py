...
# Reuse the shared prepare_chunk_indices instead of keeping a local copy.
...
def prepare_wy_repr_bwd_kernel(
    ...
    T,
    B,
    H: tl.constexpr,  # query-head count (= v/beta/g/A head count)
    Hg: tl.constexpr,  # kv-head count (k head count); Hg <= H, H % Hg == 0
    K: tl.constexpr,
    V: tl.constexpr,
    NT: tl.constexpr,
    BT: tl.constexpr,
    ...
    for idx in range(start_idx, start_idx + chunks_this_pid):
        for i_b in range(B):
            ...
            for i_h in range(0, H):
                ...
                # GQA: k indexed at Hg-stride and folded q-head -> kv-head;
                # dk written per q-head (H-stride) so the orchestrator can
                # reduce across the H/Hg group afterward.
                i_kv = i_h // (H // Hg)
                # First k-loop accumulates b_dA (and b_dbeta/b_dg) from the dw
                # path; dk's first-stage contribution is recomputed in the
                # second k-loop and combined with the second-stage contribution
                # into a single store, avoiding an in-kernel store-then-load
                # on dk that has shown stale-read behavior on some NPU runs.
                for i_k in range(tl.cdiv(K, BK)):
                    p_k = tl.make_block_ptr(
                        k + (bos * Hg + i_kv) * K, (T, K), (Hg * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                    )
                ...
                b_A_acc = tl.zeros([BT, BT], dtype=tl.float32)
                ...
                for i_k in range(tl.cdiv(K, BK)):
                    ...
                    b_k = tl.load(p_k, boundary_check=(0, 1))
                    b_dw = tl.load(p_dw, boundary_check=(0, 1))
                    # Recompute first-stage dk contribution (b_A is the
                    # originally-loaded triangular factor).
                    b_dk_beta_g = tl.dot(b_A, b_dw)
                    b_dk = b_dk_beta_g * b_beta[:, None] * b_g_exp[:, None]
                    b_k_beta = (b_k * b_beta[:, None]).to(b_k.dtype)
                    b_A_acc += tl.dot(b_k_beta, tl.trans(b_k))
...
def recompute_w_u_fwd_kernel(
    ...
    T_tmp,
    B,
    H: tl.constexpr,  # query-head count (= v/beta/g/A/w/u head count)
    Hg: tl.constexpr,  # kv-head count (k head count); Hg <= H, H % Hg == 0
    K: tl.constexpr,
    V: tl.constexpr,
    NT: tl.constexpr,
    ...
    for idx in range(start_idx, start_idx + chunks_this_pid):
        for i_b in range(B):
            for i_h in range(0, H):
                ...
                # GQA: k indexed at Hg-stride and folded q-head -> kv-head;
                # w written at H-stride (per q-head). gk follows k (per-kv-head)
                # so it's also Hg-strided.
                i_kv = i_h // (H // Hg)
                for i_k in range(tl.cdiv(K, BK)):
                    p_k = tl.make_block_ptr(
                        k + (bos * Hg + i_kv) * K, (T, K), (Hg * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                    ...
                    if USE_GK:
                        p_gk = tl.make_block_ptr(
                            gk + (bos * Hg + i_kv) * K, (T, K), (Hg * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                        )
...
def recompute_w_u_fwd(
    ...
    # GQA: k.shape[2] = Hg (kv-head count), v.shape[2] = H (q-head count).
    # When H == Hg this collapses to the no-GQA path.
    B, T, Hg, K = k.shape
    H = v.shape[-2]
    V = v.shape[-1]
    if H % Hg != 0:
    ...
    # w is per q-head (matches v's head count, NOT k's). When H == Hg, same shape as k.
...
def prepare_wy_repr_bwd(
    ...
    # dk is written per q-head (H-stride) inside the kernel; we reduce across
    # the H/Hg group at the end so the returned dk matches k's shape (Hg-head).
    # When H == Hg this is a no-op view.
    dk_int = k.new_empty(B, T, H, K) if Hg != H else torch.empty_like(k)
    ...
    # GQA reduce: each kv-head gets contributions from H/Hg q-heads. Sum
    # across the group dim so the returned dk has k's (B, T, Hg, K) shape.
    if Hg != H:
        dk = dk_int.view(B, T, Hg, H // Hg, K).sum(3)
    else:
        dk = dk_int
