...
from fla.utils import ascend_compile_kwargs, input_guard, npu_leftover_mask, npu_pad, npu_unpad
...
def recompute_w_u_fwd_kernel_npu(
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        if USE_G:
            ...
            b_g_raw = tl.load(p_g, boundary_check=(0,)).to(tl.float32)
            b_g = tl.where(m_t, exp2(b_g_raw), 0) if MASK_LEFTOVER else exp2(b_g_raw)
...
def prepare_wy_repr_bwd_da_gate_npu(
    ...
    if MASK_LEFTOVER:
        o_t = i_t * BT + tl.arange(0, BT)
        m_t = o_t < T
        b_g_diff = tl.where(m_t[:, None] & m_t[None, :], b_g[:, None] - b_g[None, :], 0)
    else:
        b_g_diff = b_g[:, None] - b_g[None, :]
    b_prod = b_dA * exp2(b_g_diff)
...
def prepare_wy_repr_bwd_npu(
    ...
    _launch_wy_core_grid(
        ...
        kernel_kwargs=dict(
            ...
            MASK_LEFTOVER=mask_leftover_fin,
            **core_base,
        ),
    )
    if use_g:
