...
def _dhu_peak_bytes(
    ...
    *,
    ...
    if leftover:
        peak += n_slabs * BK * BV * 4
    # `b_dh *= exp2(gk)[:, None]` copies each live slab.
    if use_gk:
        peak += n_slabs * BK * BV * 4
    return peak
...
def _select_bwd_dhu_tiles(
    ...
    *,
    ...
    D=100 KDA at BV=128 needs 221KiB of 192KiB UB.
    """
    if state_v_first:
        return 64, _get_bv(K, V)
...
def chunk_gated_delta_rule_bwd_dhu_npu(
    ...
    if state_v_first:
        dh = q.new_zeros(B, NT, HV, VS, KS)
        dh0_pad = q.new_zeros(h0.shape[0], HV, VS, KS, dtype=torch.float32) if h0 is not None else None
        if dht is not None:
        ...
        dh = q.new_zeros(B, NT, HV, KS, VS)
        dh0_pad = q.new_zeros(h0.shape[0], HV, KS, VS, dtype=torch.float32) if h0 is not None else None
        if dht is not None:
    ...
    dv2 = dv.new_zeros(*dv.shape[:-1], VS)
    _launch_core_grid(
