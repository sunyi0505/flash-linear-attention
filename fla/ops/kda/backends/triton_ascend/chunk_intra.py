...
_BWD_INTRA_DQ_MEM_MULT = 10.0
_BWD_INTRA_DK_MEM_MULT = 9.0
_FALLBACK_BK = 16
_MAX_BK = 64
...
def _get_bwd_intra_bk(K: int) -> int:
    return compute_row_tile_block_size(
        _BC,
        K,
        max(_BWD_INTRA_DQ_MEM_MULT, _BWD_INTRA_DK_MEM_MULT),
        tiling_row=False,
        safety_margin=_SAFETY_MARGIN,
        fallback=_FALLBACK_BK,
        min_block=16,
...
def _bwd_intra_g_base(g, bos, i_b, i_hv, T_seq, K, HV, IS_VARLEN: tl.constexpr, G_T_CONTIG: tl.constexpr):
    if G_T_CONTIG:
        ...
        return g + (i_b * HV * T_seq * K).to(tl.int64) + i_hv.to(tl.int64) * T_seq * K
...
def chunk_kda_bwd_kernel_intra_dq_db_npu(
    q, k, g, beta, dAqk, dAkk, dq, dq2, dk_part, dg_part, db,
    ...
    IS_VARLEN: tl.constexpr, SAFE_GATE: tl.constexpr, G_T_CONTIG: tl.constexpr,
):
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        dk_part_ptr = dk_part + (bos * HV + i_hv).to(tl.int64) * K
        dg_part_ptr = dg_part + (bos * HV + i_hv).to(tl.int64) * K
        ...
        for i_i in tl.static_range(NC):
            ...
            if i_ti < T_cur:
                ...
                p_b = tl.make_block_ptr(beta_ptr, (T_cur,), (HV,), (i_ti,), (BC,), (0,))
                ...
                if i_i > 0:
                    ...
                    for j_idx in tl.static_range(3):
                        i_j = j_idx
                        if i_j < i_i:
                ...
                p_dk_part = tl.make_block_ptr(dk_part_ptr, (T_cur, K), (HV * K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))
                p_dg_part = tl.make_block_ptr(dg_part_ptr, (T_cur, K), (HV * K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))
                ...
                tl.store(p_dk_part, b_dk2.to(p_dk_part.dtype.element_ty), boundary_check=(0, 1))
                tl.store(p_dg_part, b_dg_part.to(p_dg_part.dtype.element_ty), boundary_check=(0, 1))
        ...
        if i_ti < T_cur:
            ...
            if i_i < nc_eff - 1:
                ...
                for i_j in range(i_i + 1, nc_eff):
                    ...
                    p_bj = tl.make_block_ptr(beta_ptr, (T_cur,), (HV,), (i_t * BT + i_j * BC,), (BC,), (0,))
                    p_dAqk = tl.make_block_ptr(dAqk_ptr, (BT, T_cur), (1, HV * BT),
                                               (i_i * BC, i_t * BT + i_j * BC), (BC, BC), (0, 1))
                    p_dAkk = tl.make_block_ptr(dAkk_ptr, (BT, T_cur), (1, HV * BT),
                                               (i_i * BC, i_t * BT + i_j * BC), (BC, BC), (0, 1))
                    ...
                    b_dkt += tl.dot(b_dAqk.to(tl.float32), b_qg.to(tl.float32), allow_tf32=False)
                    b_dkt += tl.dot(b_dAkk.to(tl.float32), b_kbg.to(tl.float32), allow_tf32=False)
                b_dkt *= exp2(b_gn - b_g)
...
def chunk_kda_bwd_kernel_intra_dk_dg_npu(
    q, k, g, beta, dAqk, dAkk, dk, dk2, dg, dg2, dk_part, dg_part, dkt_part,
    cu_seqlens, chunk_indices, B, T, NT, BH_TOTAL, task_num, num_core,
    H: tl.constexpr, HV: tl.constexpr, K: tl.constexpr, BT: tl.constexpr,
    BC: tl.constexpr, BK: tl.constexpr, NC: tl.constexpr,
    IS_VARLEN: tl.constexpr, SAFE_GATE: tl.constexpr, G_T_CONTIG: tl.constexpr,
    ...
    for task_id in tl.range(core_id, task_num, num_core):
        ...
        if i_ti < T_cur:
            ...
            p_dg_part = tl.make_block_ptr(dg_part_ptr, (T_cur, K), (HV * K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))
            p_dkt_part = tl.make_block_ptr(dkt_part_ptr, (T_cur, K), (HV * K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))
            b_dk2 = tl.load(p_dk_part, boundary_check=(0, 1)).to(tl.float32)
            b_dg2 = tl.load(p_dg_part, boundary_check=(0, 1)).to(tl.float32)
            ...
            p_kj = k_ptr + i_ti.to(tl.int64) * H * K + o_k
            p_gkj = g_base + i_ti.to(tl.int64) * g_row_stride + o_k
            p_bj = beta_ptr + i_ti * HV
            if SAFE_GATE:
                p_gn = g_base + (i_ti + min(BC // 2, T_cur - i_ti - 1)).to(tl.int64) * g_row_stride + o_k
                b_gn = tl.load(p_gn, mask=m_k, other=0).to(tl.float32)[None, :]
                p_q = tl.make_block_ptr(q_ptr, (T_cur, K), (H * K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))
                b_q = tl.load(p_q, boundary_check=(0, 1))
                p_dAqk = tl.make_block_ptr(dAqk_ptr, (BT, T_cur), (1, HV * BT), (i_i * BC, i_ti), (BC, BC), (0, 1))
                p_dAkk = tl.make_block_ptr(dAkk_ptr, (BT, T_cur), (1, HV * BT), (i_i * BC, i_ti), (BC, BC), (0, 1))
                b_dAqk_diag = tl.load(p_dAqk, boundary_check=(0, 1)).to(tl.float32)
                b_dAkk_diag = tl.load(p_dAkk, boundary_check=(0, 1)).to(tl.float32)
...
def chunk_kda_bwd_intra_npu(
    ...
    dk_part = torch.empty_like(dk)
    dg_part = torch.empty_like(dg, dtype=torch.float)
    dkt_part = torch.empty_like(dk, dtype=torch.float)
    db2 = beta.new_empty(NK, *beta.shape, dtype=torch.float)
    dg2 = torch.empty_like(dg, dtype=torch.float)

    bh_total = B * HV
    task_num_dq = NK * NT * bh_total
    task_num_nc = NK * NC * NT * bh_total
    common_launch = dict(
        cu_seqlens=cu_seqlens,
        chunk_indices=chunk_indices,
        B=B,
