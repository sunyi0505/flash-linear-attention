...
Helpers (do not re-implement): `npu_pad`, `npu_unpad`, `npu_leftover_mask`, `npu_pad_last_dim`, `npu_pad_state_h` in `fla/utils/_ascend_align.py`, re-exported from `fla.utils`.
...
| 48 | 16 | **96** | `n % tile == 0` but `n % 32 != 0`: ceil to 32 → 64, then 48+32=80 > 64 so add another 32 |
...
`MASK_LEFTOVER` is a `tl.constexpr` and follows `n % tile != 0` (or varlen), **not** `n % 32 != 0`. When every tile is full (`n=48`, `tile=16`), leftover DMA rounded to 32 is absorbed by `npu_pad` padding — do not force leftover `tl.where` copies (and smaller Cube tiles) on that path.
