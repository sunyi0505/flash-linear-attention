...
- **Last-dim leftover DMA**: pad state last dims with `npu_pad(K, BK)` / `npu_pad(V, BV)` (`KS`/`VS`); leftover `tl.where` and in-kernel `b_h *= gk` defeat `enable_ubuf_saving`, so do not over-admit D256 BK=256/BV=128 on leftover. See [last-dim-dma-pad.md](last-dim-dma-pad.md).
...
- **Last-dim leftover DMA**: pad `VS`/`KP`/`VP` with `npu_pad`; leftover outputs are `zeros` (masked stores RMW `empty`); mask causal `g_i-g_j` **before** `exp2` so leftover `g=0` cannot overflow to inf. See [last-dim-dma-pad.md](last-dim-dma-pad.md).
